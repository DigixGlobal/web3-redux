...
export function reduxifyWeb3({ web3, networkId }) {
  const api = {};
...
export function reduxifyContract({ abi, address, web3, networkId }) {
  ...
  abi.forEach((definition) => {
    ...
    api[definition.name] = (...args) => {
      return createContractTransaction({ networkId, args, address, method: contract[definition.name] });
    };

    // hook up calls
    const callKey = ['calls', definition.name].join('.');
    api[definition.name].call = (...args) => {
      return getWeb3Method({ networkId, args, address, collection: 'contracts', key: callKey, method: contract[definition.name].call });
    };
  });
  return api;
}
