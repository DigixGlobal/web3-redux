...
export const actions = {
  ...
  NETWORK_STATUS: `${NAMESPACE} network status update`,
  ...
  return (dispatch) => {
    return new Promise((resolve, reject) => {
      method.apply(null, args.concat([(err, txHash) => {
        ...
        dispatch({ networkId, txHash, type: actions.TRANSACTION_CREATED, created: new Date() });
...
export function createContractTransaction({ args, method, address, networkId }) {
  return (dispatch) => {
    return new Promise((resolve, reject) => {
      const id = uuid();
      dispatch({ type: actions.CONTRACT_UPDATED_TRANSACTION, id, networkId, address, payload: {} });
      return createTransaction({ args, method, networkId })(dispatch)
      .then((txHash) => {
        dispatch({ type: actions.CONTRACT_UPDATED_TRANSACTION, id, networkId, address, payload: { txHash } });
        resolve(txHash);
      })
      .catch((error) => {
        dispatch({ type: actions.CONTRACT_UPDATED_TRANSACTION, id, networkId, address, payload: { error } });
        reject(error);
      });
    });
  };
